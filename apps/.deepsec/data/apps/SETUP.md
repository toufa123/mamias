# Agent setup for `apps`

This is a deepsec scanning workspace. Project `apps` was just registered
(target: `..`). Setup is incomplete — `data/apps/INFO.md`
...
2. **Fill in `data/apps/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id apps
pnpm deepsec process --project-id apps
```
